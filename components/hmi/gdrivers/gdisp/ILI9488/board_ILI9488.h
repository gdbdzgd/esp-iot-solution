...
{
    write_cmddata(g, ILI9488_COLADDRSET_REG, MAKEWORD( ((g->p.x) >> 8), (g->p.x) & 0xFF, ((g->p.x + g->p.cx - 1) >> 8), (g->p.x + g->p.cx - 1) & 0xFF));
    write_cmddata(g, ILI9488_PAGEADDRSET_REG, MAKEWORD( ((g->p.y) >> 8), (g->p.y) & 0xFF, ((g->p.y + g->p.cy - 1) >> 8), (g->p.y + g->p.cy - 1) & 0xFF));
    write_index (g, ILI9488_MEMORYWRITE_REG);
}
